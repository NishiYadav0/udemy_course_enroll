...
- [License](#license)
- [📦 Chapter 1 — Full Deployment Walkthrough (Oracle Cloud, start to finish)](#-chapter-1--full-deployment-walkthrough-oracle-cloud-start-to-finish)
- [🖥️ Chapter 2 — Front-End Setup (Telegram, Channels, Udemy — zero SSH)](#️-chapter-2--front-end-setup-telegram-channels-udemy--zero-ssh)

---

<details>
<summary><h2>📦 Chapter 1 — Full Deployment Walkthrough (Oracle Cloud, start to finish)</h2></summary>

Click to expand. Everything below assumes you're starting from nothing — no
server, no account, nothing installed. By the end you'll have the bot running
24/7 and the Admin Panel reachable in a browser. Replace every `<your-ip>`
below with your own VM's real public IP as you go.
...
[Admin Panel](#admin-panel) section for why that matters and what to do if
...
Skip the manual Telegram login here entirely — **Chapter 2 below handles
logging in to Telegram, picking channels, and adding your Udemy account
through the Admin Panel's setup wizard**, with no terminal needed for any of
it. Just get the service file in place now so it's ready to start itself the
...
configured yet. That's your cue to move on to Chapter 2.

</details>

<details>
<summary><h2>🖥️ Chapter 2 — Front-End Setup (Telegram, Channels, Udemy — zero SSH)</h2></summary>

Click to expand. Everything from here happens in the browser, at
`https://<your-ip>/` from Chapter 1 — no more terminal.
...
Dashboard takes over from here — service status, memory, live logs, and
(from the [Categories](#admin-panel) page) full control over what kind of
courses actually get enrolled.
...
> **Categories and their minimum durations are editable from the admin panel's
> Categories page (2026-08-25)** — no code edit needed. The table below still shows
> what this project ships with by default; add, edit, or delete categories from
> there instead of editing `utils/filter.py` by hand. See [Admin Panel](#admin-panel).
...
**Deploying this for someone else?** A first-run setup wizard (`/setup/claim` on a
fresh deploy) walks a new admin through creating their account, logging in to
Telegram (phone → code → optional 2FA, no SSH), picking channels by name instead of
hunting numeric chat IDs, and pasting their own Udemy cookies — ending with the bot
started automatically. Infrastructure setup (Python, nginx, systemd — see
`ORACLE_DEPLOYMENT_GUIDE.md`) is still a one-time terminal job, but no `.env` file
ever needs to be hand-edited. Full flow documented in `ADMIN_PANEL_GUIDE.md` § 9.
...
| Categories | Add, edit, or delete course categories — their keyword lists and per-category minimum duration — that used to be hardcoded in `utils/filter.py`. Seeded automatically from the built-in defaults the first time you open it. Restart required to apply, same as Environment. |
